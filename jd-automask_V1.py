...
print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++')
print('++++++++++++++++(C) 2020 cycz , Rlacat ++++++++++++++++')
print('+++++++++++https://github.com/cycz/jdBuyMask/++++++++++')
print('+++++++++https://github.com/rlacat/jd-automask+++++++++')
print('++++++++请认真阅读使用教程和配置文件，Readme.md++++++++')
print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++')
time.sleep(2)
'''
需要修改
...
#print(skuids)
if len(skuids[0]) == 0:
    logger.error('请在config.ini文件中输入你的商品id！！不会请看教程.txt或访问https://github.com/rlacat/jd-automask')
    sys.exit(1)
if area == "":
    logger.error('请在config.ini文件中输入你的地区id，否则程序无法正常运行！！不会请看教程.txt或访问https://github.com/rlacat/jd-automask')
    sys.exit(1)
'''
备用
'''
...
def auto_buy(sku_id_list):
    #参数：有货的商品列表
    threads=[]
    i=1
    for skuId in inStockSkuid:
        #print(skuId)
        t=item_process(i,"item_exist_"+str(skuId),skuId) #循环 实例化i个Thread类，传递函数及其参数，并将线程对象放入一个列表中
        threads.append(t)
        threads[i-1].start()  #循环 开始线程
        if(threading.activeCount()>thread_max_nums):#限制最多进程个数
            if not pure_mode:
                print("###[等待]已到达配置最大进程数["+str(thread_max_nums)+"]###")
            for j in range(i):
...
while (1):
    try:
        ...
        logger.info('第' + str(flag) + '次检测结束.耗时： '+str((endtime - starttime).seconds)+"s")
    except Exception as e:
        import traceback
